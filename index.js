...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.xupye.mongodb.net/myFirstDatabase?retryWrites=true&w=majority`;
...
async function run() {
  try {
    ...
    const productionCollection = client.db('powersurge').collection('supplements');
     const myCollection = client.db('powersurge').collection('selectedItems');
    ...
    //delete items
        app.delete("/additem/:id", async (req, res) => {
            ...
            const result = await productionCollection.deleteOne(filter);
            res.send(result);
        })
    ...
    app.put("/products/:id", async (req, res) => {
      ...
      $set: {
            ...
            const result = await productionCollection.updateOne(filter, updateDoc, options);
            res.send(result);
        })
